...
public class Window_BuildMenu : Window
{
    ...
    public override void DoWindowContents(Rect inRect)
    {
        ...
        //Favorited
        if (Widgets.ButtonImage(favoritesRect, BuildMenuContent.Favorite_Filled))
    ...
    private void DoDesignator(BuildableDef def, Rect rect, Vector2 size, ref Vector2 curXY)
    {
        var extension = def.GetModExtension<BuildMenuExtension>();
        if (!DebugSettings.godMode && extension.isDevOption) return;
        if (SubMenuThingDefList.IsActive(_menuDef, def))
            Designator(def, rect, size, ref curXY);
        else
            InactiveDesignator(def, rect, size, ref curXY);
    }
    ...
    private void Designator(BuildableDef def, Rect main, Vector2 size, ref Vector2 XY)
    {
        ...
        if (HighLightOptions.Contains(def))
            Widgets.DrawTextureFitted(rect, BuildMenuContent.Undiscovered, 1);
        ...
        var favTex = favorited ? BuildMenuContent.Favorite_Filled : TeleContent.Favorite_Unfilled;
