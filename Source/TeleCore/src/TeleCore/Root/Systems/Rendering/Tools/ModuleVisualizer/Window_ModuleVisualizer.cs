...
{
    private static readonly int renderResultSize = 512;
    private static readonly float nodeGraphScale = 1f;
    ...
    private readonly List<ModuleNode> allNodes = new();
    ...
    public override Vector2 InitialSize => new(1600, 800);
    ...
    private void ReadoutShaderProps(Shader shader)
    {
        ...
        for (var i = 0; i < shader.GetPropertyCount(); i++)
        {
            ...
            foreach (var attribute in shader.GetPropertyAttributes(i)) TLog.Message($" - {attribute}");
        }
    }
    ...
    private void DrawNodeGraph(Rect inRect)
    {
        ...
        //
        foreach (var node in allNodes) node.IOAnchors.DrawConnectionLine();
    ...
    public void RenderModuleResult(Rect inRect, ModuleBase result)
    {
        ...
        //
        var switch2DRect = new Rect(inRect.x - 20, inRect.y + 5, 15, 15);
        var switch3DRect = new Rect(inRect.x - 20, inRect.y + 25, 15, 15);

        TWidgets.DrawBox(switch2DRect, TColor.White075, 1);
    ...
    {
        if (FinalOutput != null) allNodes.Remove(FinalOutput);
