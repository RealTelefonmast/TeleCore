...
public class NetworkBillStack : IExposable
{
    ...
    public DefValueStack<Flow.Values.NetworkValueDef, double> TotalCost { get; set; }
    public DefValueStack<Flow.Values.NetworkValueDef, double> ByProducts { get; set; }
    ...
    public void TryCreateNewBill()
    {
        ...
        customBill.SetCost(new DefValueStack<Flow.Values.NetworkValueDef, double>(TotalCost));

        if (!ByProducts.IsEmpty)
            customBill.byProducts = new DefValueStack<Flow.Values.NetworkValueDef, double>(ByProducts);
    ...
    private void ResetBillData()
    {
        ...
        for (var i = 0; i < Ratios.Count(); i++)
        {
            ...
            TotalCost = new DefValueStack<Flow.Values.NetworkValueDef, double>();
            ByProducts = new DefValueStack<Flow.Values.NetworkValueDef, double>();
        }
    }
