...
public readonly partial struct ImmutableArray<T> : IList<T>, IList, IStructuralComparable, IStructuralEquatable, IImmutableList<T>
{
    /// <summary>
    ...
    public ImmutableArray<T> RemoveAll(Predicate<T> match)
    {
        ...
        return removeIndices != null ? self.RemoveAtRange(removeIndices) : self;
    }
    ...
    public ImmutableArray<T> RemoveRange(ReadOnlySpan<T> items, IEqualityComparer<T>? equalityComparer = null)
    {
        ...
        if (items.Length == 1)
        {
            return Remove(items[0], equalityComparer);
        }
        ...
        for (var i = 0; i < items.Length; i++)
        {
            ...
            do
            {
                index = IndexOf(items[i], index + 1, equalityComparer);
            } while (index >= 0 && !indicesToRemove.Add(index) && index < Length - 1);
        }

        return RemoveAtRange(indicesToRemove);
    }
